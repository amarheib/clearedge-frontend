    ...
    <header className="sticky top-0 z-40 backdrop-blur bg-white/70 border-b border-slate-200">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="hidden md:inline text-xs px-2 py-1 rounded-full bg-emerald-100 text-emerald-700">MVP v0.3-lite</span>
        </div>
      </div>
    </header>
  );
  ...
  return (
    <section className="text-center" id="how">
      ...
      <p className="mt-3 text-slate-600 max-w-2xl mx-auto">
        העלה חשבונית, קבל דו"ח QuickFix צבעוני: מה הבעיה, ולמה זה נחסם, ומה לתקן.
      </p>
    </section>
  );
}
...
function runTests(): TestResult[] {
  ...
  try { const csv='supplierVat,customerVat,total,vat,date,currency,invoiceId\n512345679,598765431,1000,170,2025-09-12,ILS,INV-2025-00123'; const obj=csvToInvoice(csv); const ok = obj.supplierVat==='512345679' and obj.total==1000; results.push({ name:'csvToInvoice parse', pass: ok, details: JSON.stringify(obj)});} catch(e:any){ results.push({ name:'csvToInvoice parse', pass:false, details:e?.message}); }
  return results;
}
